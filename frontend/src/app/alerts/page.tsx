    ...
    const fetchAlerts = async (searchTerm = "") => {
        try {
            ...
            if (showIncidentRelated) {
                // Use the dedicated incident-related API
                const incidentAlerts = await AlertService.getAlertsUnderIncident(
                    true, 
                    sortField, 
                    sortOrder
                );
                ...
                response = await AlertService.searchAlerts(
                    searchTerm, 
                    limit, 
                    offset, 
                    sortField, 
            ...
            <div className="bg-white shadow rounded-lg h-full flex flex-col">
                <div className="px-6 py-4 border-b border-gray-200">
                    ...
                    <div className="flex space-x-4">
                        ...
                        {/* INCIDENT RELATED FILTER */}
                        <button
                            onClick={toggleIncidentRelated}
                            className={`px-3 py-1 text-sm rounded-md flex items-center space-x-1 ${
                                showIncidentRelated ? "bg-indigo-50 text-indigo-600" : "text-gray-600 hover:bg-gray-100"
                            }`}
                        >
